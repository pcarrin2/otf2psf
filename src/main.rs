...
fn main() -> Result <(), Box<dyn error::Error>> { 
    ...
    let ttf_parser = ttf_parser::TtfParser::from_font_and_charset_paths(
        &cli_options.otf_file,
        &cli_options.unicode_table_file,
        &cli_options.height,
    )?;

    let (unicode_table, glyph_count, glyphs) = match &cli_options.unicode_table_file {
        Some(p) => (
            unicode_table::UnicodeTable::from_file(p, &cli_options.glyph_count)?,
            Some(unicode_table.data.len() as u32),
            psf2_writer::Psf2GlyphSet::new_with_unicode_table(ttf_parser, unicode_table)?,
        ),
        None => (
            None,
            if let Some(n) = &cli_options.glyph_count {n} else { 256 },
            psf2_writer::Psf2GlyphSet::new(ttf_parser, glyph_count)?,
        ),
    };
